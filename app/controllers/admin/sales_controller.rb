class Admin::SalesController < ApplicationController
  ...
  def graph      
      ...
      get_size    
        ...
        flash[:warning] = ('Bad range, from '+start_date.to_date.to_s+' to '+end_date.to_date.to_s+' are no data, change range please.')
  ...
  def get_data    
    #if user_id
     #@sale = Sale.all(:conditions => { :created_at => start_date..end_date },
     #    :joins => :sales_products, :select => "sales.*, sum(sales_products.count) as real_counts",  
     #    :order =>  "created_at").group_by{ |sale| sale.created_at.strftime("(%y/%d/%m)") }         
         #@sale.each do |sale| puts sale.sales_products.count end 
    #end
    #if pack_id
  ...
  def start_date
    begin
      ...
      @start_date = DateTime.civil(params[:et][:"end_date(1i)"].to_i, 
